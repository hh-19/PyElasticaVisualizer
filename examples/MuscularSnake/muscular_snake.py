...
for idx in range(len(post_processing_dict_list)):
    post_processing_dict[str(idx)] = post_processing_dict_list[idx]

if SAVE_DATA:

    import pickle
    with open("examples/MuscularSnake/muscular_snake.dat", "wb") as f:
        pickle.dump(post_processing_dict, f)

############################### Visualizing ###############################

if VISUALIZE:

    visualization_dict = generate_visualization_dict(post_processing_dict)
...
plot_video_with_surface(
    post_processing_dict_list,
    video_name="examples/MuscularSnake/muscular_snake.mp4",
    fps=rendering_fps,
    step=1,
...
plot_snake_velocity(
    post_processing_dict_list[0], period=period, filename="examples/MuscularSnake/muscular_snake_velocity.png"
)
